...
SELECT CASE
    WHEN dummy_encode_sql('cats', 'y') = 'SELECT * EXCLUDE ("g"), ("g" = ''b'')::INT AS "g_b" FROM query_table(''cats'')'
...
-- as the explicitly selected gradient-descent solver.
...
SELECT CASE
    WHEN (SELECT max(abs(g.coefficient - r.coefficient))
          FROM poisson_fit('irdat','y', solver := 'gd') g JOIN poisson_fit('irdat','y', solver := 'irls') r USING (feature)) < 1e-5
     AND (SELECT max(abs(g.coefficient - r.coefficient))
          FROM linreg_fit('irdat','y', solver := 'gd') g JOIN linreg_fit('irdat','y', solver := 'irls') r USING (feature)) < 1e-6
