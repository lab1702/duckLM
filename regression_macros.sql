-- ============================================================================
-- Logistic & linear regression as pure DuckDB (>= 1.5) SQL macros.
--
-- Public macros:
...
--                            a provably convergent default (4/(d+1) logistic,
--                            1/(d+1) linear)
...
__reg_namecheck AS (
    SELECT CASE
             ...
             ELSE true
           END AS ok
    FROM (SELECT CAST(COLUMNS(*) AS VARCHAR) FROM query_table(tbl) LIMIT 1)
         UNPIVOT INCLUDE NULLS (v FOR colname IN (COLUMNS(*)))
),
...
-- linear so the optimization and tol are scale-free in y.
__reg_ystats AS (
    SELECT CASE WHEN family = 'logistic' THEN 0.0 ELSE avg(v) END AS mu_y,
...
__reg_cfg AS (
    SELECT CASE
             ...
             -- sigmoid derivative bound) or L <= d+1 (linear).
...
__reg_gd AS (
    ...
    FROM (
        SELECT it, betas, look,
               -- beta_j <- look_j + step * (1/n) * sum_i xs_ij * r_i
               list_transform(look, lambda b, j:
                   b + step * list_sum(list_transform(res, lambda ob: ob.xs[j] * ob.r)) / n) AS newbetas
        FROM (
            SELECT it, betas, n, step, look,
...
FROM (
    SELECT '(Intercept)' AS feature,
           ys.mu_y + ys.sd_y * (s.betas[1] - coalesce(list_sum(list_transform(f.names,
               lambda nm, j: s.betas[j + 1] * f.mus[j] / f.sigmas[j])), 0.0)) AS coefficient
    FROM __reg_sol s, __reg_feats f, __reg_ystats ys
    UNION ALL
    SELECT unnest(f.names),
           unnest(list_transform(f.names, lambda nm, j: ys.sd_y * s.betas[j + 1] / f.sigmas[j]))
    FROM __reg_sol s, __reg_feats f, __reg_ystats ys
)
...
__reg_namecheck AS (
    SELECT CASE
             ...
             WHEN bool_or(starts_with(lower(colname), '__reg_'))
               THEN error(caller || ': column names beginning with "__reg_" are reserved for internal use; please rename')
             WHEN caller = 'logit_predict' AND bool_or(lower(colname) IN ('prob', 'pred'))
             ...
             WHEN caller = 'linreg_predict' AND bool_or(lower(colname) = 'prediction')
               THEN error('linreg_predict: the input table already has a "prediction" column, which collides with the output column; rename or drop it first (e.g. SELECT * EXCLUDE (prediction))')
             ELSE true
           END AS ok
    FROM (SELECT CAST(COLUMNS(*) AS VARCHAR) FROM query_table(tbl) LIMIT 1)
         UNPIVOT INCLUDE NULLS (v FOR colname IN (COLUMNS(*)))
